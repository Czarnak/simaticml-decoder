...
def test_precedence_not_binds_tightest_or_parenthesised():
    val = ir.And([ir.VarRef("#a"), ir.Not(ir.VarRef("#b")),
                  ir.Or([ir.VarRef("#c"), ir.VarRef("#d")])])
...
def test_or_wraps_and_groups_for_readability():
    val = ir.Or([ir.And([ir.VarRef("#a"), ir.VarRef("#b")]),
                 ir.And([ir.VarRef("#c"), ir.VarRef("#d")])])
    assert "(#a AND #b) OR (#c AND #d)" in _scl_of(ir.Assign(target=ir.VarRef("#x"), value=val))


def test_negated_assign():
    s = _scl_of(ir.Assign(target=ir.VarRef("#x"), value=ir.VarRef("#y"),
                          kind=ir.AssignKind.NEGATED))
    assert "#x := NOT #y;" in s


def test_reset_coil_emits_if_guard():
    s = _scl_of(ir.Assign(target=ir.VarRef("#x"), value=ir.VarRef("#c"),
                          kind=ir.AssignKind.RESET))
    assert "IF #c THEN" in s and "#x := FALSE;" in s and "END_IF;" in s


def test_latch_is_called_out():
    s = _scl_of(ir.Assign(target=ir.VarRef("#x"),
                          value=ir.Or([ir.VarRef("#start"), ir.VarRef("#x")]),
                          is_latch=True, note="seal-in latch: #x feeds back"))
    assert "seal-in latch" in s


def test_flipflop_reset_priority():
    s = _scl_of(ir.FlipFlop(target=ir.VarRef("#q"),
                            set_expr=ir.VarRef("#s"), reset_expr=ir.VarRef("#r"),
                            reset_priority=True))
...
def test_box_move_assignment_form():
    s = _scl_of(ir.BoxCall(instruction="Move", inputs={"in": ir.Literal("0")},
                           outputs={"out1": ir.VarRef("#c")}))
    assert "#c := 0;" in s


def test_box_add_with_enable_guard():
    s = _scl_of(ir.BoxCall(instruction="Add",
                           inputs={"in1": ir.VarRef("#x"), "in2": ir.Literal("10")},
                           outputs={"out": ir.VarRef("#x")}, enable=ir.VarRef("#e")))
...
def test_box_inc_under_rising_edge():
    ...
    s = _scl_of(ir.BoxCall(instruction="Inc", inputs={"operand": ir.VarRef("#c")},
                           enable=edge))
    assert "IF R_TRIG(#alarm) THEN" in s and "#c := #c + 1;" in s


def test_box_timer_instance_call_form():
    s = _scl_of(ir.BoxCall(instruction="TON",
...
def test_user_call_fc():
    s = _scl_of(ir.UserCall(name="deviceState", block_type="FC",
                            params={"Alarm": ir.VarRef("#a")}))
...
def test_scl_has_block_and_network_headers_and_trailing_newline():
    ...
    block.networks.append(ir.NetworkLogic(
        index=1, language="LAD", title="Net One",
...
def test_sidecar_schema_interface_and_xref():
    interface = model.Interface(sections=[
        model.Section(name="Input", members=[
    ...
    assert set(side) >= {"block", "interface", "networks", "xref",
                         "instruction_inventory", "warnings", "trace"}
...
def test_sidecar_trace_maps_statement_uid():
    block = ir.DecodedBlock(name="B", kind="FB", interface=model.Interface())
    block.networks.append(ir.NetworkLogic(index=3, language="LAD", statements=[
        ir.Assign(target=ir.VarRef("#x"), value=ir.VarRef("#y"), uid="40")]))
    side = emit.emit_sidecar(block)
